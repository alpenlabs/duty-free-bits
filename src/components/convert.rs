...
/// Bulk version of [`hot_to_ring`].
///
/// Evaluates `S` affine maps `a_j · g(x) + b_j` in ring R from a single OHE of
/// x, sharing one [`ohe_scale_bulk`] call across all S outputs. Equivalent in
/// semantics to `S` independent `hot_to_ring` calls.
///
/// When the coefficients are NCF, the bulk OHE step registers per-`h[i]`
/// switch groups (see [`ohe_scale_bulk`]), packing the per-position S switches
/// into a single wide CCRH call.
pub fn hot_to_ring_bulk(
    ...
    for j in 0..s_dim {
        ...
        assert_eq!(
            sys.is_cf(a_vec[j]),
            sys.is_cf(b_vec[j]),
            "hot_to_ring_bulk: a_j and b_j must share kind"
        );
    }
    ...
    for j in 0..s_dim {
        let mut result = sys.constant_matching(0, a_vec[j]);
